...
DEMON_LAUGH = pygame.mixer.Sound('Assets_devil_laugh.mp3')
DEMON_DYING = pygame.mixer.Sound('Assets_demon-slaughter.mp3')
DEMON_FIRE = pygame.mixer.Sound('Assets_DEMON_FIRE.mp3')
...
COWBOY_IMAGE = pygame.image.load(os.path.join( 'cowboy.png'))
COWBOY = pygame.transform.scale(COWBOY_IMAGE, (50, 90))

DEMON_IMAGE = pygame.image.load(os.path.join( 'demon.png'))
DEMON = pygame.transform.scale(DEMON_IMAGE, (100,150))

SPACE = pygame.transform.scale(pygame.image.load(os.path.join( 'Tileset4.png')), (WIDTH, HEIGHT))
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LSHIFT and len(cowboy_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(cowboy.x + cowboy.width, cowboy.y + cowboy.height//2 - 2, 10, 5)
                    cowboy_bullets.append(bullet)
                    BULLET_FIRE_SOUND.play()

                if event.key == pygame.K_RSHIFT and len(demon_bullets) < MAX_BULLETS:
